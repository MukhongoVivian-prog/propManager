...
class AddPropertyForm(forms.Form):
    ...
    property_type = forms.ChoiceField(
        ...
        widget=forms.Select(attrs={'class': 'form-control'}) # Add a class for styling
    )

    description = forms.CharField(
        label="Description",
        widget=forms.Textarea(attrs={'placeholder': 'Describe your property', 'rows': 4}) # Multi-line input
    )
    ...
    property_images = forms.FileField(
        ...
        widget=forms.ClearableFileInput(attrs={'multiple': False}), # Allows selecting multiple files
