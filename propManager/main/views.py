...
def landlord_dashboard(request):
    properties = Property.objects.filter(landlord=request.user).annotate(bookings_count=Count('bookings'))
...
def landlord_bookings(request):
    ...
    if request.method == 'POST':
        ...
        booking = Booking.objects.filter(id=booking_id, property__in=properties).first()
        if booking:
            if action == 'approve':
...
def reserve_booking(request, booking_id):
    ...
    if booking.status == 'pending':
        ...
        messages.success(request, f'Booking for {booking.property.title} has been reserved!')
...
def process_payment(request, booking_id):
    ...
    if booking.status == 'approved' and request.method == 'POST':
        ...
        messages.success(request, 'Payment completed successfully! You can now check in.')
...
def checkin_booking(request, booking_id):
    ...
    if booking.status == 'payment_completed':
        ...
        messages.success(request, f'Successfully checked in to {booking.property.title}!')
        return redirect('booking_success', booking_id=booking.id)
    else:
...
def properties(request):
    ...
    properties = Property.objects.filter(status='Available')[:6]  # Show first 6 available properties
